...
public class UTWCIngameEditor : MonoBehaviour {
	...
	void ShowControls(int id) {
		...
		GUILayout.BeginHorizontal();
		Color color = UTWC.renderer.material.GetColor("_Color");
		...
		if (colorString != color.ToString()) {
			UTWC.renderer.material.SetColor("_Color",color);
		}
		GUILayout.EndHorizontal();

		GUILayout.BeginHorizontal();
		float startTC = UTWC.renderer.material.GetFloat("_ColorBias");
		float cloneTC = startTC;
		GUILayout.Label("Texture/Color:" ,GUILayout.Width(100));
		startTC = GUILayout.HorizontalSlider(startTC,0f,1f);
		if (cloneTC != startTC) {
			UTWC.renderer.material.SetFloat("_ColorBias",startTC);
		}
		GUILayout.EndHorizontal();

		GUILayout.BeginHorizontal();
		float startTT = UTWC.renderer.material.GetFloat("_TextureBias");
		float cloneTT = startTT;
		GUILayout.Label("Texture/Texture:" ,GUILayout.Width(100));
		startTT = GUILayout.HorizontalSlider(startTT,0f,1f);
		if (cloneTT != startTT) {
			UTWC.renderer.material.SetFloat("_TextureBias",startTT);
		}
		GUILayout.EndHorizontal();

		GUILayout.BeginHorizontal();
		float startT1S = UTWC.mainTextureTitle.x;
		...
		GUILayout.BeginHorizontal();
		float startHF = UTW.renderer.material.GetFloat("_HeightFactor");
		...
		if (cloneHF != startHF) {
			UTW.renderer.material.SetFloat("_HeightFactor",startHF);
		}
		GUILayout.EndHorizontal();

		GUILayout.BeginHorizontal();
		float startSD = UTW.renderer.material.GetFloat("_HighlightThresholdMax");
		float cloneSD = startSD;
		GUILayout.Label("Shore Foam:" ,GUILayout.Width(100));
		startSD = GUILayout.HorizontalSlider(startSD,0f,10f);
		if (cloneSD != startSD) {
			UTW.renderer.material.SetFloat("_HighlightThresholdMax",startSD);
		}
		GUILayout.EndHorizontal();

		GUILayout.BeginHorizontal();
		Color color2 = UTW.renderer.material.GetColor("_Color2");
		...
		if (color2String != color2.ToString()) {
			UTW.renderer.material.SetColor("_Color2",color2);
		}
		GUILayout.EndVertical();
		GUILayout.EndHorizontal();

		GUILayout.BeginHorizontal();
		float startSDC = UTW.renderer.material.GetFloat("_HighlightThresholdRange");
		...
		if (cloneSDC != startSDC) {
			UTW.renderer.material.SetFloat("_HighlightThresholdRange",startSDC);
		}
		GUILayout.EndHorizontal();
